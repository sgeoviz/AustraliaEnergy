...
import geoJson2 from "./Postcode.json";
...
const Handling = () => {
  ...
  const filteredpost = geoJson2.filter((data) => {
    return (
      (!selectedState || data.properties.State === selectedState) &&
      (!selectedstation || data.properties.Type === selectedstation) &&
      (!selectedYear || data.properties.year <= selectedYear)
  ...
  const Statefilter = () => {
    return (
      <div>
        ...
        <button
          className="button"
          onClick={() => filterstate("ACT")}
          style={selectedState === "ACT" ? { backgroundColor: "blue" } : {}}
        >
          ACT
        </button>
        <button
  ...
  const Stationfilter = () => {
    return (
      <div id="station-filter">
        ...
        <button
          className="button"
          onClick={() => filterstation("Rooftop Solar")}
          style={
            selectedstation === "Rooftop Solar"
